...
impl ShaderComs {
    ...
    pub fn new(device: &ID3D11Device, dir: &String) -> Result<Self, MyErr> {
        // Open vshader.cso here for input layout creation
        let mut vshader_bytebuf = Vec::new();
        File::open(dir.clone() + "vshader.cso")
            .map_err(|_| MyErr::d3d(EKnd::Io, "Open vshader.cso failed"))?
            .read_to_end(&mut vshader_bytebuf)
            .map_err(|_| MyErr::d3d(EKnd::Io, "Read vshader.cso failed"))?;
        ...
        let vshader = unsafe {
            device
                ...
                .map_err(|_| MyErr::d3d(EKnd::Creation, "vshader"))?
        };
        // Pixel shader
        let pshader = unsafe {
            let mut buf = Vec::new();
            File::open(dir.clone() + "pshader.cso")
                .map_err(|_| MyErr::d3d(EKnd::Io, "Open pshader.cso failed"))?
                .read_to_end(&mut buf)
                .map_err(|_| MyErr::d3d(EKnd::Io, "Read pshader.cso failed"))?;
            let bytecode = buf.as_ptr() as *const _ as *const ::core::ffi::c_void;
            device
                .CreatePixelShader(bytecode, buf.len(), None)
                .map_err(|_| MyErr::d3d(EKnd::Creation, "pshader"))?
        };
        // Input layout
        let ilayout = unsafe {
            ...
            device
                ...
                .map_err(|_| MyErr::d3d(EKnd::Creation, "InputLayout"))?
        };
        // Constant buffer
        let cbuffer = unsafe {
            ...
            device
                ...
                .map_err(|_| MyErr::d3d(EKnd::Creation, "Cbuffer"))?
        };
        Ok(Self {
            vshader,
            pshader,
